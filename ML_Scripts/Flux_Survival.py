...
grid_search = GridSearchCV(
    ...
    cv=cv.split(TCGA_flux_input, tissue_labels),  # Use Stratified Shuffle Split
    n_jobs=-1  # Use all available cores
)

grid_search.fit(TCGA_flux_input, OS_array)
...
cv_results_rf.to_csv('flux_input/cv_results_flux_rxns_RF.csv', index = True)

best_model_rf = grid_search.best_estimator_
import pickle

model_pkl_file = "flux_input/RF_flux_rxns.pkl"
...
result = permutation_importance(best_model_rf, TCGA_flux_input, OS_array, n_repeats=3, random_state=random_state)
fimportance = pd.DataFrame(
    ...
    index=TCGA_flux_input.columns,
).sort_values(by="importances_mean", ascending=False)

fimportance.to_csv('flux_input/feature_importance_flux_RF.csv', index = True)
...
coxnet_pipe.fit(TCGA_flux_input, OS_array)
...
gcv = GridSearchCV(
    ...
    scoring=ipcw_scorer,
    cv=cv.split(TCGA_flux_input, tissue_labels),
    error_score=0.5,
    n_jobs=-1,
).fit(TCGA_flux_input, OS_array)

cv_results_cox = pd.DataFrame(gcv.cv_results_)

best_model_Cox = gcv.best_estimator_
best_coefs_Cox = pd.DataFrame(best_model_Cox.coef_, index=TCGA_flux_input.columns, columns=["coefficient"])
...
cv_results_cox.to_csv('flux_input/cv_results_flux_rxns_COX.csv', index = True)
import pickle
model_pkl_file = "flux_input/COX_flux_rxns.pkl"
with open(model_pkl_file, 'wb') as file:
    pickle.dump(best_model_Cox, file)

best_coefs_Cox.to_csv('flux_input/feature_importance_pathway_rxns_COX.csv', index = True)
...
gcv.fit(TCGA_flux_input, OS_array)
...
cv_results_gbsa.to_csv('flux_input/cv_results_flux_rxns_GBSA.csv', index = True)
import pickle
model_pkl_file = "flux_input/GBSA_flux_rxns.pkl"
with open(model_pkl_file, 'wb') as file:
    pickle.dump(best_model_gbsa, file)

best_coefs_gbsa.to_csv('flux_input/feature_importance_pathway_rxns_GBSA.csv', index = True)
...
cv_results_svm.to_csv('flux_input/cv_results_flux_rxns_SVM.csv', index = True)
import pickle
model_pkl_file = "flux_input/SVM_flux_rxns.pkl"
with open(model_pkl_file, 'wb') as file:
    pickle.dump(best_model_svm, file)

best_coefs_svm.to_csv('flux_input/feature_importance_pathway_rxns_SVM.csv', index = True)
### Kernel Support Vector Machine
from sksurv.kernels import clinical_kernel
from sksurv.svm import FastKernelSurvivalSVM

kernel_matrix = clinical_kernel(TCGA_flux_input)
...
best_coefs_ksvm = pd.DataFrame(best_model_ksvm.coef_, best_model_ksvm.feature_names_in_)

cv_results_ksvm.to_csv('flux_input/cv_results_flux_rxns_KSVM.csv', index = True)
import pickle
model_pkl_file = "flux_input/KSVM_flux_rxns.pkl"
with open(model_pkl_file, 'wb') as file:
    pickle.dump(best_model_ksvm, file)

best_coefs_ksvm.to_csv('flux_input/feature_importance_pathway_rxns_KSVM.csv', index = True)